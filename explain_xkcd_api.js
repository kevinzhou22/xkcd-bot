...
// Searches the explain XKCD wiki and returns the first valid comic's title number as a string. Returns -1
// if the search does not find anything
const searchExplainXkcd = async function searchExplainXkcd(keywords) {
  ...
  for (let i = 0; i < search.length; i++) {
    ...
    const startsWithNumber = colonSearch !== -1 && (Number(searchEntry.slice(0, colonSearch)) !== NaN);
